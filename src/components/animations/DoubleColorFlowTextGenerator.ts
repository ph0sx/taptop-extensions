...
import doubleColorFlowTextStyles from './DoubleColorFlowTextGenerator.styles.css';
...
interface DoubleColorFlowFormData extends GeneratorConfig {
...
interface DoubleColorFlowConfig {
  rules: DoubleColorFlowRule[];
}

// Чистые типы без ID для генерации кода
type CleanDoubleColorFlowRule = Omit<DoubleColorFlowRule, 'id'>;

interface CleanDoubleColorFlowConfig {
  rules: CleanDoubleColorFlowRule[];
}

@customElement('double-color-flow-text-generator')
export default class DoubleColorFlowTextGenerator extends BaseGenerator {
  static styles = [...BaseGenerator.styles, unsafeCSS(doubleColorFlowTextStyles)];

  // Lit состояние
  @state() private accessor config: DoubleColorFlowConfig = {
    rules: [
      {
        id: this.generateId(),
        animationIdentifier: 'js-double-color-flow-1',
        animationSpeed: '0.3s',
        colorFrom: '#7F7F7F',
        colorTo: '#664EE8',
  ...
  private validationController = new ReactiveValidationController<DoubleColorFlowConfig>(this, {
  ...
  protected getValidationRules(): ValidationRuleMap {
    ...
    this.config.rules.forEach((rule) => {
      ...
      // Правила для промежуточного цвета
      rules[`color-to-${rule.id}`] = [
        { type: 'required', message: 'Промежуточный цвет обязателен' },
        {
          type: 'pattern',
          message: 'Цвет должен быть в формате HEX (#RRGGBB)',
          options: { pattern: /^#[0-9A-Fa-f]{8}$/ },
   ...
   */
  private extractFieldValue(state: DoubleColorFlowConfig, fieldId: string): string {
  ...
  private updateConfig(newConfig: DoubleColorFlowConfig): void {
    this.config = newConfig;
  }

  // Методы для управления правилами
  private addRule(): void {
    const newRule: DoubleColorFlowRule = {
      id: this.generateId(),
      animationIdentifier: `js-double-color-flow-${this.config.rules.length + 1}`,
      animationSpeed: '0.3s',
      colorFrom: '#7F7F7F',
      colorTo: '#664EE8',
      slowdownEffect: false,
  ...
  private updateRule(
    id: string,
    field: keyof Omit<DoubleColorFlowRule, 'id'>,
  ...
  private renderRuleCard(rule: DoubleColorFlowRule, index: number): TemplateResult {
        ...
        <div class="rule-block">
          ...
          <div class="rule-main">
            <ttg-input
              ...
              placeholder="js-double-color-flow"
            ...
            <ttg-color-picker
              ...
              ''}"
              tooltip="Цвет текста в начале анимации и после возврата в формате HEX"
              @update-value="${(e: CustomEvent) => {
            ...
            <ttg-color-picker
              id="color-to-${rule.id}"
              label="Промежуточный цвет"
              .value="${rule.colorTo}"
              required
              .error="${this.validationController.getFieldState(`color-to-${rule.id}`)?.error ??
              ''}"
              tooltip="Цвет текста в середине анимации, на который будет переход и с которого возврат в формате HEX"
              @update-value="${(e: CustomEvent) => {
            ...
            <ttg-input
              ...
              .error="${this.validationController.getFieldState(`animation-speed-${rule.id}`)
                ?.error ?? ''}"
              tooltip="Длительность одного перехода цвета (прямого или обратного). Рекомендуется 0.3s"
              @update-value="${(e: CustomEvent) => {
  ...
  protected collectData(): DoubleColorFlowFormData | null {
  ...
  private removeIdsFromConfig(config: DoubleColorFlowConfig): CleanDoubleColorFlowConfig {
    return {
  ...
  protected generateCode(settings: DoubleColorFlowFormData): string {
    // Parse back from serialized format
    const config: DoubleColorFlowConfig = {
  ...
  .${animationIdentifier}-child.first-animation {
  ...
  .${animationIdentifier}-child.second-animation {
    ...
    (function() {
      ...
      function animateElement(element) {
        ...
        function createAndAnimateCharacters() {
          ...
          const animationDelay = inputAnimationDelay;
          let delay = 0;
          ...
          setTimeout(() => {
            ...
            charElements.forEach((charSpan, index) => {
              const position = index / (charElements.length - 1 || 1);
              let totalSlowdownFactor = 0;

              secondDelay += animationDelay * (0.5 + totalSlowdownFactor);
