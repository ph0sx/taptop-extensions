...
import fadeInTextStyles from './FadeInTextGenerator.styles.css';
...
interface FadeInTextFormData extends GeneratorConfig {
...
interface FadeInTextConfig {
  rules: FadeInTextRule[];
}

// Чистые типы без ID для генерации кода
type CleanFadeInTextRule = Omit<FadeInTextRule, 'id'>;

interface CleanFadeInTextConfig {
  rules: CleanFadeInTextRule[];
}

@customElement('fade-in-text-generator')
export default class FadeInTextGenerator extends BaseGenerator {
  static styles = [...BaseGenerator.styles, unsafeCSS(fadeInTextStyles)];

  // Lit состояние
  @state() private accessor config: FadeInTextConfig = {
    rules: [
      {
        id: this.generateId(),
        animationIdentifier: 'js-text-animation-fade-in-1',
        animationSpeed: '0.3s',
        animationDelay: 0.03,
        delayBeforeStart: '0s',
  ...
  private validationController = new ReactiveValidationController<FadeInTextConfig>(this, {
   ...
   */
  private extractFieldValue(state: FadeInTextConfig, fieldId: string): string {
  ...
  private updateConfig(newConfig: FadeInTextConfig): void {
    this.config = newConfig;
  }

  // Методы для управления правилами
  private addRule(): void {
    const newRule: FadeInTextRule = {
      id: this.generateId(),
      animationIdentifier: `js-text-animation-fade-in-${this.config.rules.length + 1}`,
      animationSpeed: '0.3s',
      animationDelay: 0.03,
      delayBeforeStart: '0s',
      slowdownEffect: false,
  ...
  private updateRule(
    id: string,
    field: keyof Omit<FadeInTextRule, 'id'>,
  ...
  private renderRuleCard(rule: FadeInTextRule, index: number): TemplateResult {
        ...
        <div class="rule-block">
          ...
          <div class="rule-main">
            <ttg-input
              ...
              placeholder="js-text-animation-fade-in"
            ...
            <ttg-input
              ...
              .error="${this.validationController.getFieldState(`animation-speed-${rule.id}`)
                ?.error ?? ''}"
              tooltip="Общая длительность анимации появления каждого символа. Рекомендуется 0.3s"
              @update-value="${(e: CustomEvent) => {
            ...
            <ttg-input
              ...
              .error="${this.validationController.getFieldState(`animation-delay-${rule.id}`)
                ?.error ?? ''}"
              tooltip="Время паузы перед появлением следующего символа. Рекомендуется 0.03"
              @update-value="${(e: CustomEvent) => {
  ...
  protected collectData(): FadeInTextFormData | null {
  ...
  private removeIdsFromConfig(config: FadeInTextConfig): CleanFadeInTextConfig {
    return {
  ...
  protected generateCode(settings: FadeInTextFormData): string {
    // Parse back from serialized format
    const config: FadeInTextConfig = {
  ...
  @keyframes ${animationIdentifier} {
    ...
    100% { opacity: 1; }
      ...
      // JavaScript код для правила - используем IIFE паттерн как в рабочей версии
      scriptsParts.push(`
    (function() {
